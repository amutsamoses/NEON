...
export const updateDriver = async (c: Context) => {
  ...
  try {
    ...
    if (foundDriver == undefined) return c.text("user not found!👽", 404);
...
export const driverWithOrder = async (c: Context) => {
  try {  
  const data = await driverWithOrderService();
  if (data == null) {
    return c.text("no user found!😶‍🌫️👽", 404);
  }
  return c.json(data, 200);
