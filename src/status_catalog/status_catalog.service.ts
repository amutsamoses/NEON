...
export const getStatusCatalogService = async (): Promise<
  TSStatusCatalog[] | null
> => {
  const statusCatalog = await db.select().from(StatusCatalog);
...
export const createStatusCatalogService = async (
  statusCatalog: TIStatusCatalog
) => {
...
export const updateStatusCatalogService = async (
  id: number,
  statusCatalog: TIStatusCatalog
) => {
  await db
